...
finalSave = () => {
  ...
  return;
    ...
    this.props.Stage3Submit(this.state.eventTime, this.state.eventDate, this.state.payByDate, this.state.eventPricePP, this.state.message, this.state.eventPlace, this.state.minInvitees)
  }

  canBeSubmitted() {
  const { eventPlace, eventDate, eventTime, eventPricePP, payByDate, minInvitees } = this.state;
  return (
    eventPlace.length > 0 &&
    eventDate.length > 0 &&
    eventTime.length > 0 &&
  ...
  render() {
    ...
    return (
      ...
      <div><label htmlFor="minInvitees">Minimum Participants</label><input type="number" name='minInvitees' id='minInvitees' required onChange={this.handleChange}/></div>
